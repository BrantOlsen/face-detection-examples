...
def run():
    ...
    logging.info("Training FaceNet...")
    trainer2 = FaceNetFaceIdentityTrainer()
    trainer2.train(faces, labels)
    
    test_identifier = FaceNetFaceIdentifier()
    logging.info("Checking the FaceNet identifier...")
    for i in range(len(faces)):
        label, conf, all_class = test_identifier.classify(faces[i])
        if label != labels[i]:
